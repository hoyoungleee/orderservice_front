...
// 요청용 인터셉터
// 인터셉터의 use함수는 매개값 두개 받습니다. 둘다 콜백 함수 형식입니다.
// 1번쨰 콜백에는 정상 동작 로직, 2번째 콜백에는 과정 중 에러 발생시 실행할 함수
axiosInstance.interceptors.request.use(
  (config) => {
    //요청보내기 전에 항상 처리에해할 내용 콜백으로 전달.
...
axiosInstance.interceptors.response.use(
  ...
  async (error) => {
    ...
    if (error.response.status === 401) {
      console.log('응답상태 401발생! 토큰 재발급 필요!');
      try {
        const id = localStorage.getItem('USER_ID');
        const res = await axios.post('http://localhost:8181/user/refresh', {
          id,
        });
        const newToken = res.data.result.token; // axios는 json()안씀
        localStorage.setItem('ACCESS_TOKEN', newToken); // 동일한 이름으로 토큰담기(덮어씀)
        ...
        // axiosInstance를 사용하여 다시한번 원본 요청을 보내고, 응답은 원래 호출한곳으로 리턴
        return axiosInstance(originalRequest);
      } catch (error) {
        console.log(error);
        //백엔드에서 401을 보낸거 -> Refresh도 만료된 상황 (로그아웃처럼 처리해줘야 함.)
        localStorage.clear();
        // 재발급 요청도 거절당하면 인스턴스를 호출한 곳으로 에러리턴
        return Promise.reject(error);
      }
    }
  },
